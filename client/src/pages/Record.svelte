<script lang="ts">
  ...
  import { getChatRecord } from './../helper/aside'
...
<section class="w-full p-2 mx-auto my-4 overflow-scroll chat-list bg-gradient-to-b from-gray-50">
  {#if chatRecordArr && chatRecordArr.length > 0}
    {#each chatRecordArr as item}
      {#if item.from === 'user'}
        <UserChat params={item} hidden={true} />
      {:else}
        <GptChat params={item} />
      {/if}
    {/each}
  {:else}
    <div class="flex flex-col justify-center w-full text-gray-600">
      <div
        role="status"
      ...
      <p class="mt-4">
        暂时没有获取到聊天记录，或许您没有开启「保存对话内容」，可以前往 「<a href="#/setting"
          >设置</a
        >」修改。
      </p>
    </div>
